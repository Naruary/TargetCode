...
*                   SPI interface.
*       @file       Downhole/src/CommDrivers/CommDriver_SPI.c
*       @date       July 2013
...
void SPI_InitPins(void)
{
	...
	GPIO_InitStructure.GPIO_Pin  = DATAFLASH_CS_PIN | FRAM_CS_PIN;
	GPIO_Init(DATAFLASH_CS_PORT, &GPIO_InitStructure);

	GPIO_WriteBit(DATAFLASH_CS_PORT, DATAFLASH_CS_PIN, Bit_SET);
	GPIO_WriteBit(FRAM_CS_PORT, FRAM_CS_PIN, Bit_SET);
...
//	GPIO_PinAFConfig(SPI_DATA_PORT, SPI_SCK, GPIO_AF_SPI1);
//	GPIO_PinAFConfig(SPI_DATA_PORT, SPI_MISO, GPIO_AF_SPI1);
//	GPIO_PinAFConfig(SPI_DATA_PORT, SPI_MOSI,  GPIO_AF_SPI1);
}

...
*       @file       Uphole/src/DataFields/UI_TimeField.c
*       @author     Chris Walker
*       @date       December 2014
...
char* TimeFormat(RTC_TimeTypeDef* now)
{
    ...
    sprintf(sBuffer, "%02d:%02d:%02d", now->RTC_Hours, now->RTC_Minutes, now->RTC_Seconds);
    return sBuffer;
}
...
static RECT* GetCharacterRect(MENU_ITEM* item, U_BYTE index)
{
    ...
    U_BYTE offset = UI_GetTextSize(BeginString(TimeFormat(&item->time.time), index + item->time.position));
    U_BYTE xSize = UI_GetTextSize(BeginString(TimeFormat(&item->time.time), index + item->time.position + 1));

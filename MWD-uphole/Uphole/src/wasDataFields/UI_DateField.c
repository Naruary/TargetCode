...
*       @file       Uphole/src/DataFields/UI_DateField.c
*       @author     Chris Walker
*       @date       December 2014
...
{
    static char sBuffer[9];
    sprintf(sBuffer, "%02d/%02d/%02d", date->RTC_Month, date->RTC_Date, (date->RTC_Year % 100));
    return sBuffer;
}
...
static RECT* GetCharacterRect(MENU_ITEM* item, U_BYTE index)
{
    ...
    U_BYTE offset = UI_GetTextSize(BeginString(DateFormat(&item->date.date), index + item->date.position));
    U_BYTE xSize = UI_GetTextSize(BeginString(DateFormat(&item->date.date), index + item->date.position + 1));
...
{
    char string[3], strValue[3];
    U_BYTE newValue;

    sprintf(string, "%02d", *value);
    if (position == 0)
    {
        sprintf(strValue, "%1d0", digit);
    }
    else
    {
        sprintf(strValue, "%c%1d", string[0], digit);
    }
...
void DateFinishEdit(MENU_ITEM* item)
{
    ...
    while (TRUE)
    {
        ...
        if (item->date.date.RTC_Year == tempDate.RTC_Year && item->date.date.RTC_Month == tempDate.RTC_Month

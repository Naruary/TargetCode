...
typedef enum { PCDT_STATE_IDLE, PCDT_STATE_SEND_INTRO, PCDT_STATE_SEND_LABELS1,
...
typedef enum{ PCDTU_STATE_FILE_IDLE, PCDTU_STATE_AWAIT_BEGIN_CSV , PCDTU_STATE_FILE_RETRIEVAL,
...
void PCPORT_UPLOAD_StateMachine(void)
{
    ...
    switch (RetrieveLogFromPC_state)
    {
      ...
      case PCDTU_STATE_AWAIT_BEGIN_CSV:
            ...
            n = UART_ReceiveMessage(CLIENT_PC_COMM, (U_BYTE*)uart_message_buffer, sizeof(uart_message_buffer) - 1);
            
            if(n > 0)
            {
                uart_message_buffer[n] = '\0';
                // ECHO
      ...
      case PCDTU_STATE_FILE_RETRIEVAL:
            n = UART_ReceiveMessage(CLIENT_PC_COMM, (U_BYTE*)csv_buffer + csv_buffer_index, CSV_BUFFER_SIZE - csv_buffer_index - 1);
            if (n > 0)
              {
